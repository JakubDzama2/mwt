...
export class FilmsListComponent implements OnInit, AfterViewInit {
  ...
  expandedColumns = ['expandedDetail'];
  expandedFilm: Film;
  ...
  expandFilm(film: Film) {
    this.expandedFilm = this.expandedFilm === film ? null : film;
  }
...
class FilmsDataSource implements DataSource<Film> {
  ...
  setObservables(paginator: MatPaginator, filter$: Observable<string>, sort: MatSort) {
    ...
    this.futureObservables.next(sort.sortChange.pipe(
      tap((sortEvent: Sort) => {
        console.log("sort event: "+ JSON.stringify(sortEvent));
        this.paginator.firstPage();
  ...
  connect(): Observable<Film[]> {
    return this.futureObservables.pipe(
      mergeAll(),
      switchMap(event => this.filmsServerService.getFilms(this.indexFrom, this.indexFrom + this.pageSize, this.filter, this.orderBy, this.descending)
        .pipe(
          map(response => {
